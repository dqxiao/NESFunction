...
#   def stable(self):
#    return self.vals[-1]-self.vals[-2]>-1*np.max(self.vals)+np.mean(self.vals)

#   def update(self,val):
#     self.vals.append(val)
#     if len(self.vals)>2*self.d:
#       self.vals.pop()


#   def diffPositive(self):
#     if len(self.vals)<self.d:
#       return False 

#     return self.vals[-1]>self.vals[-2]

#   def longEnough(self):
#     return len(self.vals)>self.d
#   def clear(self):
#     self.vals=[]
...
class PEPGVariant:
  ...
  def __init__(self, num_params,             # number of model parameters
    ...
    self.sigmaWindow=SlideWindow(num_params)  # adaptive diversity function 
  ...
  def tell(self, reward_table_result):
    ...
    self.sigmaWindow.update(np.square(calProduct(self.sigma)))



    #using reward history to dynamic control the diversity weights   
    # self.rewardWindow.append(np.array(reward))  
    # if len(self.rewardWindow)>5:
    #   self.rewardWindow.pop(0) 
    ...
    if (self.sigma_alpha > 0):
      ...
      #just start from nothing 
      # if len(self.rewardWindow)==1:
      #   self.diversity_best=0 
      # else:
      #   #thanks---to--the simple idea 
      # if self.rewardWindow.reverse():
      #   self.diversity_best= min(0.1,0.8*abs(self.rewardWindow.diffVal()/self.sigmaWindow.diffVal()+5))
      #   self.rewardWindow.clear()
      #   self.sigmaWindow.clear()  
      # else:
      #   self.diversity_best=0
     
      # self.diversity_best=0 
      # if self.rewardWindow.longEnough() and self.rewardWindow.stable() and self.sigmaWindow.diffPositive():
      #   self.diversity_best=0.005
      #   self.rewardWindow.clear()
      #   self.sigmaWindow.clear()
      # if self.rewardWindow.longEnough() and not self.rewardWindow.stable() and not self.sigmaWindow.diffPositive():
      #   self.diversity_best=4
      #   self.rewardWindow.clear()
      #   self.sigmaWindow.clear()
  ...
  def tell(self, reward_table_result):
    ...
    if (self.sigma_alpha > 0):
      ...
      if self.rewardWindow.evident():
        ...
        if self.rewardWindow.lastDiff()<0 or self.rewardWindow.lastDiff<self.rewardWindow.std():
            self.diversity_best = 0
      self.sigma += self.sigma*self.learning_rate*self.diversity_best
      self.sigma += change_sigma
